...
- /store_data 
    ...
    > Sample POST: data (list), tag (str), description (str)
        ...
        curl -i -H "Content-Type: application/son" -X POST -d '{"data":[1,2,3,4,5,12], "tag": "utility", "description": "my 2021 utility bills"}' http://localhost:5001/store_data
...
Open a terminal session and run:
		...
		curl -i -H "Content-Type: application/son" -X POST -d '{"idx": 0, "computation":"max", "name": "max netfix Payment", "reason": "I need to know the max netflix  payment"}' http://localhost:5002/request_data
...
Leveraging this conda environment will install all the necessary dependencies and libraries to run the API endpoints above. 
